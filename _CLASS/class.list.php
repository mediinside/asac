...
CLASS ListClass
{
	...
	function getPageLink()
	{
		...
		{			
			...
			$rst ='<a href="?page='  . $prev . '&' . $tail . '" title="이전으로 이동하기" class="btn prev ntxt"><span>이전 목록</span></a> ';
			for($i = $start ; $i <= $end ; $i++){
				$pageLastclass = "";
				if ($i>=10) {
			...
			$rst .= ' <a href="?page=' . $this -> page_info['next'] . '&'. $tail . '" title="다음 페이지 이동하기" class="btn next ntxt"><span>다음 목록</span></a>';
