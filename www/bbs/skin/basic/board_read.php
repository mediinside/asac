...
<div class="board_view">
 ...
 <div class="btn-group">
   <div class="btn-lt">
      <?php
				//글목록버튼
				echo "<a href=\"#\" onclick=\"javascript:location.href='${index_page}?jb_code=${jb_code}&${search_key}&search_keyword=${search_keyword}&page=${page}'\" class=\"btn btn-middle btn-default\" title='목록'>목록</a>";	
				?>
    </div>
   <div class="btn-rt">
      <?
			...
			if($check_level >= 9 || $check_id == $jb_mb_id)
					echo "<a href=\"#\" onclick=\"javascript:location.href='${get_par}&jb_mode=tmodify'\" class=\"btn btn-middle btn-default \" title=\"수정\">수정</a> ";
			//삭제권한(쓰기권한이 있어야 삭제 가능)
			if($check_level >= 9 || $check_id == $jb_mb_id)
					echo "<a href=\"#\" onclick=\"javascript:location.href='${get_par}&jb_mode=tdelete'\" class=\"btn btn-middle btn-gray \" title=\"삭제\">삭제</a> ";								
			//쓰기권한
			if($check_level >= $db_config_data['jba_write_level'])
					echo "<a href=\"#\" onclick=\"javascript:location.href='${index_page}?jb_code=${jb_code}&jb_mode=twrite'\" class=\"btn btn-middle btn-gray \" title=\"쓰기\">쓰기</a>";						
			?>	
    </div>
  </div>
  <?php
