...
if($check_level < $db_config_data['jba_read_level']) {
	if($jb_code == "20" && $_SESSION['suserid']) {
		$C_Func->put_msg_and_back("고대민동 내부자료가 포함되어 있어 CMS회원만 볼 수 있습니다. ");
		die;
	}else{
		$C_Func->put_msg_and_go("해당게시판의 읽기권한이 없습니다.", "/member/login.html?re_url=".$GP -> NOWPAGE);
		die;
	}
}
