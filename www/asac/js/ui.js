...
co.init = function(body, version){
	...
	if(co.device == 'mb'){
		...
		mainLabel.children('.i-text').on({
			'focus focusin' : function(){
				$(this).parent().children('.main-holder').hide();
			},
			'blur focusout' : function(input, placeholder){
				input = $(this),
				placeholder = $(this).parent().children('.main-holder');
				if (input.val().length > 0){
					placeholder.hide();
				} else {
					placeholder.show();
		...
		mainLabel.each(function(eq,label){
			label = $(label);
			var input = label.children('.i-text'),
				placeholder = label.children('.main-holder');
			if (input.val().length > 0){
				placeholder.hide();
			} else {
				placeholder.show();
	...
	}else if(co.device == 'tb' || co.device == 'pc'){
		var mainLabel = $('.main-label'),
			placeholder = mainLabel.children('.main-holder');
			placeholder.show();

		mainLabel.children('.i-text').on({
			'focus focusin' : function(){
				$(this).parent().children('.main-holder').show();
			},
			'blur focusout' : function(input, placeholder){
				input = $(this),
				placeholder = $(this).parent().children('.main-holder');
