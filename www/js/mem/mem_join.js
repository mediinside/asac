$(document).ready(function(){		
		...
		$('#mb_pwd_ok').alphanumeric({allow:"/^.*(?=^.{8,15}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/"});
		$('#mb_mobile2').numeric();
		$('#mb_mobile3').numeric();								   
		...
		$('#frmJoin').validate({
			...
			messages: {	
				...
				mb_mobile1: { required: "핸드폰 번호를 선택하세요"},
				...
				mb_pwd: { required: "패스워드를 입력하세요", minlength: $.format("패스워드는 {0}자 이상입니다"), maxlength: $.format("패스워드는 {0}자 이하입니다."), alphanumeric : $.format("영문(소)과 숫자를 조합하세요.")},
